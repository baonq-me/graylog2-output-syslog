...
import com.baonq.graylog2.gelfclient.GelfMessageBuilder;
import com.baonq.graylog2.gelfclient.GelfMessageLevel;
import com.baonq.graylog2.gelfclient.GelfTransports;
...
public class JsonLogOutput implements MessageOutput {

	private static final Logger LOG = LoggerFactory.getLogger(JsonLogOutput.class);
	...
	@Inject
	public JsonLogOutput(@Assisted Configuration configuration) throws MessageOutputConfigurationException {
		this(buildTransport(configuration));
	}

	@VisibleForTesting
	JsonLogOutput(GelfTransport gelfTransport) {
	...
	protected static GelfTransport buildTransport(final Configuration configuration) throws MessageOutputConfigurationException {
		...
		try {
			...
			final String error = "Error initializing " + JsonLogOutput.class;
			LOG.error(error, e);
	...
	protected GelfMessage toGELFMessage(final Message message) {
		...
		final String forwarder = JsonLogOutput.class.getCanonicalName();
	...
	public interface Factory extends MessageOutput.Factory<JsonLogOutput> {
		@Override
		JsonLogOutput create(Stream stream, Configuration configuration);
	...
	public static class Descriptor extends MessageOutput.Descriptor {
		public Descriptor() {
			super("Custom Json Output", false, "", "An output sending GELF over TCP or UDP");
		}
	}
}
